...
# -----------------------------
# Step 0: Ask for project preferences
# -----------------------------
# If a value was already passed in as a command-line parameter
# (e.g. .\SETUP.ps1 -ProjectName CoreProject), that value is used as-is
# and the prompt for it is skipped. Otherwise you're asked interactively,
# and pressing Enter without typing anything falls back to the default
# shown in [brackets].
Write-Host "===== Django Project Setup =====" -ForegroundColor Magenta
Write-Host "Press Enter to accept the default shown in [brackets]." -ForegroundColor DarkGray
Write-Host ""

if (-not $RootFolder) {
    $inputValue = Read-Host "Root folder name [ProjectFolder]"
    $RootFolder = if ($inputValue) { $inputValue } else { "ProjectFolder" }
}

if (-not $ProjectName) {
    $inputValue = Read-Host "Django project name [myProject]"
    $ProjectName = if ($inputValue) { $inputValue } else { "myProject" }
}

if (-not $AppName) {
    $inputValue = Read-Host "Django app name [myApp]"
    $AppName = if ($inputValue) { $inputValue } else { "myApp" }
}
